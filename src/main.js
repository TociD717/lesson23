...
class CalorieCalculator {
  constructor() {
    // Внутреннее хранилище для продуктов, реализованное через Map
    this.products = new Map();
  }

  // Метод для добавления продукта с его калорийностью
  addProduct(productName, productCalories) {
    this.products.set(productName, productCalories);
  }

  // Метод для получения калорийности продукта по названию
  getProductCalories(productName) {
    if (this.products.has(productName)) {
      return this.products.get(productName);
    } else {
      return 'Product not found'; // Если продукт не найден
    }
  }

  // Метод для удаления продукта по названию
...
class UniqueUsernames {
  ...
  exists(username) {
    return this.nameList.has(username);
  }
...
console.log (uniqueUsernames.nameList);
uniqueUsernames.addUser('john_doe');
